...
description: Calculate FE/BE engineering timeframe from a Jira Story per SP/day.
  Weight-aware assignment: heavy tickets to senior, supporting to junior.
  Remaining SP, preview, optional Jira date+assignee update.
  Triggers on "buat timeframe dari story", "generate timeline story",
  "hitung jadwal pengerjaan story", or "buat jadwal pengerjaan story".
---
...
| Field | ID |
|---|---|
| `Story Point[Number]` / `Story Point` | `customfield_10024` |
| `Story Point Dev[Number]` / `Story Point Dev` | `customfield_10123` |
| `Story point estimate` | `customfield_10016` |
| `Expected Start Date[Date]` | `customfield_10195` |
| `Expected End Date[Date]` | `customfield_10196` |
...
for f in fields:
    ...
    if name in ['story point', 'story point dev', 'expected start date', 'expected end date']:
        print(f['id'], f['name'])
"
```
